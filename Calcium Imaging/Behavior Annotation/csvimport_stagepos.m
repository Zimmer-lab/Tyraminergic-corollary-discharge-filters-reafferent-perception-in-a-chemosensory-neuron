function varargout = csvimport_stagepos( fileName, varargin )
...
%get rid of single diguts which come from faulty deparation:
lv=NaN;
for ci=1:length(rowData)
    lv(ci)=length(rowData{ci});
end
rowData(lv<2)=[];
nCols           = numel( rowData );
...
for ii = 2 : nLines
    ...
    rowDataCells       = regexp( rowData, p.delimiter, 'split' );
    lv=NaN;
    for ci=1:length(rowDataCells)
        lv(ci)=length(rowDataCells{ci});
    end
    rowDataCells(lv<2)=[];
    
    if ~isempty (strfind(rowDataCells{1},'ch')) | ~isempty(strfind(rowDataCells{2},'ch')) | ~isempty(strfind(rowDataCells{2},'pa'))% only cherry data!
        
        nDataElems    = numel( rowDataCells );
        if nDataElems < nCols
            ...
            data(cc,1:length(rowDataCells))    = rowDataCells;
            cc=cc+1;
            continue
        elseif nDataElems > nCols
            ...
            %find corresponding data fields:
            fi=NaN(1,length(rowDataCells));
            try
                if isempty(data{cc-1,1})
                    ccc=1;
                    while isempty(data{cc-ccc,1}) &ccc <20000
                        ccc=ccc+1;
                    end
                    idx=ccc;
                else
                ...
                for jj=1:length(rowDataCells)
                    if jj<=length(data(cc-idx,:))
                        if ceil(length(data{cc-idx,jj})/5)==ceil(length(rowDataCells{jj})/5) || strcmp(rowDataCells{jj}(1:4),data{cc-idx,jj}(1:4))
                            fi(jj)=1;
                        end
                    end
                end
